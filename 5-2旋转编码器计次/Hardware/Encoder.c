...
void Encoder_Init(void)
{
	...
	NVIC_InitStructure2.NVIC_IRQChannel=EXTI1_IRQn;
	NVIC_InitStructure2.NVIC_IRQChannelCmd=ENABLE;
	NVIC_InitStructure2.NVIC_IRQChannelPreemptionPriority=1;
	NVIC_InitStructure2.NVIC_IRQChannelSubPriority=2;
	NVIC_Init(&NVIC_InitStructure2);
}
int16_t Encoder_Get(void) //返回变化值
{
...
{
	if(EXTI_GetITStatus(EXTI_Line0)==SET)  //检测中断位
	{
...
{
	if(EXTI_GetITStatus(EXTI_Line1)==SET)  //检测中断位
	{
		if(GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_0)==0)
		{
			Encoder_Count++;
		}
		EXTI_ClearITPendingBit(EXTI_Line1);  //清除中断位
	}
}
